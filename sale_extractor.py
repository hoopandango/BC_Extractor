...
def load_item_pack():
	with open(flnames_jp["itempack"]) as fl:
...
def load_localisable():
	with open(flnames_en["localisable"],encoding = 'utf-8') as fl:
		df_en = pd.read_csv(fl,header = None,delimiter = '\t')
	...
	with open(flnames_jp["localisable"],encoding = 'utf-8') as fl:
		df_jp = pd.read_csv(fl,header = None,delimiter = '\t')
