...
def invert_imp():
	...
	with open('_config.json',mode ='w') as fl:
		json.dump(config,fl,indent='\t')
...
def get_combo_sizes_table()->DataFrame:
	with open(flnames_en['sizes']) as fl:
		tsze = pd.read_csv(fl,header = None,delimiter = '|')
...
def get_combo_effects_table()->DataFrame:
	with open(flnames_en['effects']) as fl:
		teff = pd.read_csv(fl,header = None,delimiter = '|')
...
def get_combo_units_table()->DataFrame:
	...
	if LNG == 'en':
		with open(flnames_en['data']) as fl:
			df = pd.read_csv(fl,header = None,delimiter = ',',usecols = range(0,15))
	elif LNG == 'jp':
		with open(flnames_jp['data']) as fl:
			df = pd.read_csv(fl,header = None,delimiter = ',',usecols = range(0,15))
...
def run():
	...
	if CBMODE and not (LNG == 'jp' and not LOADMODE):
		...
		print('Data converted to CatBot schemas')
